\chapter{Methode Softwareplattform}
\label{ch:MethodeSoftwareplattform}

In diesem Kapitel wird anhand der Anforderungen aus dem Forschungsansatz eine Gesamtarchitektur der Softwareplattform vorgestellt, sowie die Softwarearchitektur der beiden Softwarekomponenten \enquote{Runtime Node} und \enquote{Runtime Master}. 

\section{Verteilte Rechenplattform aus autonomen Fahrzeugsteuergeräten}

Die Rechenleistung von autonomen Fahrzeugen kann in verschiedenen Nutzungskonzepten verwendet werden. In \autoref{Hardwareumgebung in Fahrzeugen} wurde die Hardware vorgestellt, die typischerweise in autonomen Fahrzeugsteuergeräten zu finden ist. Es wird deutlich, dass diese Steuergeräte über Hardwaremodule verfügen, die die Rechenleistung für spezifische Rechenoperationen erhöhen. Grafikprozessoren beinhalten viele Rechenkerne, die parallel auf großen Datenmengen Operationen ausführen können. In den meisten Architekturen können die GPU-Cores nur in Gruppen verwendet werden, wobei jede Gruppe die gleiche Rechenoperation ausführen muss. Kerne für Neuronale Netzwerk Berechnungen implementieren Matrixoperationen in Hardware, so dass diese in erheblich weniger Rechenzyklen als bei sequentieller Berechnung durchgeführt werden können. Folglich können nur solche Rechenaufgaben sinnvoll auf diese Steuergeräte ausgelagert werden, die sich in viele parallele Berechnungen aufteilen lassen. Typische Beispiele für solche Berechnungen sind:

\begin{itemize}
    \item Mathematik: Vektor- und Matrixoperationen
    \item Bildbearbeitung: FFT oder Faltungsoperationen
    \item Simulation: Moleküldynamik, Finite-Elemente Analyse, Fluiddynamik
    ...
    \item Optimierung: Genetische Algorithmen Berechnungen
    \item Quantencomputing: Simulation von Quantumsystemen
\end{itemize}
...
\begin{itemize}
    \item Nutzung ausschließlich durch den Hersteller: Diese Methode ist die transparenteste und sicherste Methode für den Fahrzeughersteller. Nur der Hersteller selber darf Applikationen auf die Fahrzeuge ausrollen und die ungenutzte Rechenleistung nutzen
    \item Nutzung durch Hersteller und ausgewählte Kunden: Der Hersteller stellt die Rechenleistung auch an ausgewählte Kunden zur Verfügung, hierbei vertraut der Hersteller den Kunden, dass sie keine illegalen Aktivitäten ausüben und nicht aktiv versuchen Schutzmechanismen des Systems umzugehen.
    \item Nutzung durch frei verfügbaren Marktplatz: Beliebige Kunden können auf dem Marktplatz Rechenleistung einkaufen. In diesem Konzept muss sichergestellt werden dass Kunden keine illegale Aktivitäten ausüben und dass sie die Sicherheitsmechanismen der Plattform nicht versuchen zu umgehen.
\end{itemize}
...
Das Konzept sieht die Implementierung eines Markplatzes vor, wo Benutzer Rechenleistung einkaufen und verkaufen können. Die Erstellung der Rechenaufgaben kann auf zwei verschiedene Arten erfolgen. Entweder werden bereits kompatible Funktionscodes in Form von Vorlagen genutzt, oder die Funktionscodes werden vom Benutzer implementiert. Die Aufgabenverwaltung registriert die neue Aufgabe, welche durch einen Benutzer angestoßen wurde. Der Benutzer, der die Rechenleistung der Plattform in Anspruch nehmen möchte, erstellt ein Angebot, in dem er die Höhe der Vergütung angibt. Diese Angebote werden auf dem Netzwerk an alle Benutzer übertragen, die Rechenleistung zur Verfügung stellen. Zusätzlich kann das Angebot Einschränkungen bezüglich der Hardware oder geographische Lage beinhalten.

Die Bereitstellung von Rechenleistung erfolgt durch die lokalen Ausführungen eines Transaktionssystem-Moduls durch die jeweiligen Benutzer. Dieses Modul sammelt alle verfügbaren Aufgabenangebote im Netzwerk und wählt das beste Angebot aus. Des Weiteren verfügt auch derjenige, der ein Angebot erstellt, über eine Reputation auf der Plattform. Angebote von Benutzern, deren Reputation als zu gering erachtet wird, werden abgelehnt. Sofern alle Anforderungen des Angebots durch den Rechenleistung-Bereitsteller erfüllt sind, wird das Angebot angenommen. 

Die Rechenaufgabe, inklusive benötigter Ressourcen wird auf den PC des Rechenleistungsbereitstellers heruntergeladen. Der PC berechnet im Anschluss die entsprechende Aufgabe. Der Rechenleistung-Bereitsteller kann die geladenen Anwendungen in verschiedenen Umgebungen ausführen. Es kann eine Virtualisierungsmethode wie Container oder virtuelle Maschine verwendet werden, um die Anwendungen vom Hostsystem zu isolieren, oder es kann direkt auf dem Hostsystem ausgeführt werden, wenn die Anwendung als vertrauenswürdig eingestuft wird. 
...
Die elementare Aufgabe der Softwarekomponente Runtime Node ist die Erzeugung und Verwaltung von isolierten Laufzeitumgebungen. In \autoref{Virtualisierung} wurde festgelegt, dass diese isolierten Laufzeitumgebungen mit der Virtualisierungsmethode der Containerisierung erzeugt werden. Dabei werden betriebssystemspezifische Isolationsmethoden verwendet, die vom Linux-Kernel zur Verfügung gestellt werden. Konkret werden die in \autoref{sub:SoftwareVirtualisierung/Container} vorgestellten Namespaces verwendet. 
...
\begin{itemize}
    ...
    \item Eine möglichst hohe Flexibilität bezüglich der Ausführungsumgebung und des Ausführungsortes ist vorteilhaft.
\end{itemize}
...
\begin{itemize}
    ...
    \item MessagePack: Binärformat, ähnliche Struktur wie \gls{JSON} aber nicht von Menschen lesbar, kompakt
...
\begin{table}
    ...
    \begin{tabular}{|c||c|c|c|c|c|}
        ...
        \hline
        Menschenlesbar & Ja & Ja & Nein & Nein & Ja \\
        \hline
...
Da Fahrzeugnetzwerke durch eine hohe Volatilität gekennzeichnet sind, beispielsweise durch Verbindungsabbrüche oder das Abschalten der Zündung, muss die Plattform über Mechanismen zur Fehlererkennung und -behandlung verfügen. Das Konzept sieht hierfür einen Heartbeat-Mechanismus vor.

Jeder Runtime Node sendet periodisch (Intervall $t_{hb}$) eine \enquote{Node Info}-Nachricht an den Runtime Master. Der Master speichert den Zeitstempel der letzten Nachricht $t_{last}$ in der Node-Datenbank. Ein Überwachungsmodul im Master prüft zyklisch die Verfügbarkeit der Knoten anhand eines definierten Timeout-Schwellwerts $t_{timeout}$. Ein Knoten $n$ gilt als ausgefallen (offline), wenn:
\begin{equation}
    t_{current} - t_{last}(n) > t_{timeout}
\end{equation}
Wird ein Ausfall detektiert, greift die folgende Wiederherstellungsstrategie (\textit{Retry Logic}):
\begin{enumerate}
    \item Der Status des Knotens wird in der Datenbank auf \enquote{offline} gesetzt.
    \item Alle diesem Knoten zugewiesenen Aufgaben, die noch nicht als \enquote{abgeschlossen} gemeldet wurden, werden als \enquote{fehlgeschlagen} markiert.
    \item Die fehlgeschlagenen Aufgaben werden zurück in die globale Warteschlange (\textit{Task Pool}) verschoben.
    \item Der Scheduler wird getriggert, um diese Aufgaben im nächsten Zyklus an andere, verfügbare Knoten neu zu verteilen.
\end{enumerate}
Dieser Mechanismus garantiert, dass Aufgaben auch bei abruptem Ausfall von Fahrzeugen nicht verloren gehen, sondern lediglich verzögert ausgeführt werden.

\subsection{Sicherheitskonzept und Authentifizierung}
Um unberechtigten Zugriff auf die Steuergeräte zu verhindern, implementiert die Plattform ein tokenbasiertes Authentifizierungsverfahren. Bei der Initialisierung generiert der Runtime Master ein kryptografisches Token (Shared Secret). Dieses Token muss jedem Runtime Node bei der manuellen Kopplung oder der sicheren Erstkonfiguration übergeben werden.
Jede Nachricht enthält im Header dieses Token. Empfängt ein Runtime Node einen Befehl, vergleicht er das empfangene Token mit dem lokal gespeicherten. Stimmen diese nicht überein, wird der Befehl ignoriert und ein Sicherheitsvorfall protokolliert. Dies verhindert, dass nicht autorisierte Teilnehmer im selben Netzwerk Drittanbieter-Code auf den Fahrzeugen ausführen können.
...
\begin{itemize}
    ...
    \item Die Ressourcenverteilung soll so effizient wie möglich erfolgen, Runtime Nodes müssen dem Runtime Master entsprechend relevante Informationen über ihre aktuell verfügbaren Rechenressourcen mitteilen.
...
Auch die Verteilung von Anwendungen in einem verteilten Computersystem kann als 0/1-Knapsack-Problem aufgefasst werden, da die einzelnen Anwendungen in der Regel nicht aufgeteilt werden können. Zur Lösung von Knapsack-Problemen existieren verschiedene Algorithmen. Grundsätzlich kann zwischen Algorithmen unterschieden werden, die eine exakte Optimallösung als Ergebnis garantieren und solchen, die nur eine lokale Optimallösung garantieren. 
...
Die dynamische Programmierung ist eine weit verbreitete Methode zur Bestimmung der optimalen Lösung des Knapsack-Problems. Bei dieser Methode wird eine Tabelle aller Möglichkeiten für Gewicht/Ressourcenverbrauch und Anzahl der Objekte erstellt. In dieser Tabelle werden alle zulässigen Kombinationen berechnet und die Kombination mit dem höchsten Wert ermittelt. Nachfolgend ist die Logik der dynamischen Programmierung als Pseudocode dargestellt. Im dargestellten Fall werden Objekte mit unterschiedlichen Gewichten und Werten betrachtet, wobei die Kombination der Objekte gesucht wird, deren Gesamtgewicht die maximale Kapazität nicht überschreitet und deren Gesamtwert maximal ist:
...
\begin{align}
    & O(2^n) \text{(exponentiell)} \\
    & O(nW) \text{(polynomisch)}
\end{align}
...
\begin{algorithmic}
    ...
    \State $n \leftarrow$ \text{Länge von} $Items$  % \text{} für Wörter in Formeln
    \For{$Maske \leftarrow 0$ \text{bis} $2^n - 1$}
        \State $Wert \leftarrow 0$, $Gewicht \leftarrow 0$
        \For{$i \leftarrow 0$ \text{bis} $n - 1$}
    ...
    \EndFor
    \State \Return $Mengen$  % Hier war der Fehler (\turn -> \Return)
    \EndFunction
...
\begin{align}
    & O(n \cdot W) \text{(pseudo-polynomiell)}
    \label{EQ:DP}
\end{align}
    Meet-in-the-Middle: \\
\begin{align}
    & O(2^{(n/2)} \cdot log(2^{(n/2)})) \text{(exponentiell (Basiswurzel halbiert))}
    \label{EQ:MitM}
\end{align}
...
\begin{align}
    & O(n \cdot log(n)) \text{(logarithmisch-linear)}
\end{align}
...
Diese Skalierung reduziert die Komplexität des Problems auf Kosten der Genauigkeit des Endergebnisses. Den Zusammenhang zwischen Fehlerfaktor und Komplexität zeigt der folgende Formel:
...
\subsubsection{Multi-Knapsack-Problem}

Das klassische 0/1-Knapsack-Problem beschreibt die Situation, dass ein Rucksack verschiedene Objekte enthalten kann. Dies entspricht dem Fall, dass verschiedene Anwendungen auf einem System mit begrenzter Ressourcenkapazität berechnet werden müssen. Verteilte Systeme bestehen hingegen aus mehreren Rechensystemen, auf denen die Anwendungen nach verschiedenen Kriterien verteilt werden müssen. Dies kann als Multi-Knapsack-Problem aufgefasst werden, bei dem es sowohl mehrere Rucksäcke als auch mehrere Optimierungskriterien gibt und nicht nur einen Maximalwert bei gegebener Kapazität. 

Multi-Knapsack-Probleme beschreiben zunächst den Fall, dass mehrere Knapsäcke vorhanden sind, also nicht nur eine optimale Lösung für eine einzelne maximale Kapazität gefunden werden soll, sondern eine optimale Verteilung auf mehrere Rucksäcke mit gegebenen Kapazitäten. Multi-Knapsack-Probleme können mit den in den vorangegangenen Abschnitten vorgestellten Algorithmen nach entsprechender Anpassung gelöst werden.
...
\begin{align}
    & O(2^n) (0/1-Knapsack) \rightarrow O((m+1)^n) \text{(Multi-Knapsack)}
\end{align}

Die Anzahl der Lösungen wächst exponentiell mit der Anzahl der Knapsäcke. Daher werden diese Probleme nur selten und nur für kleine $ n $ und $ m $ auf diese Weise gelöst. Eine effizientere Methode bietet die dynamische Programmierung, die in \autoref{Exakte Algorithmen} für das 0/1-Knapsack-Problem vorgestellt wurde. Für Multi-Knapsäcke wird die Tabelle mit jedem zusätzlichen Knapsack um eine Dimension erweitert, so dass für alle Gewichtsverteilungen in den Rucksäcken die besten Werte in die Tabelle eingetragen werden. Der Berechnungsaufwand steigt bei dieser Methode ebenfalls exponentiell mit der Anzahl an Knapsäcken $ m $:

\begin{align}
    & O(n \cdot W) \text{(0/1-Knapsack)} \rightarrow O(n \cdot W_1 \cdot W_2 \cdot ... \cdot W_m) \text{(Multi-Knapsack)}
\end{align}
...
\begin{itemize}
    ...
    \item Evolutionäre Algorithmen: optimierungsbasierte Alogrithmen, die an dem Prinzip der biologischen Evolution orientieren. 
    \item Dynamische Programmierung für Multi-Objective Knapsack-Probleme: Die Tabelle kann erweitert werden dass für jede Kapazität nicht nur ein Wert gespeichert wird, sondern alle nicht dominierte Lösungskombinationen.
...
\begin{table}
    ...
    \begin{tabular}{m{5cm} m{2cm} m{2cm} m{2cm} m{2cm}}
        ...
        Evolutionäre Algorithmen & \usym{2717} & \usym{2713} & \usym{2713} & \usym{2713} \\
    ...
    \end{tabular} 
    \caption{Vergleich der häufig verwendeten Methoden für Multiobjektive Knapsack-Probleme}
\end{table}

Zur Lösung von Scheduling-Problemen in verteilten Rechensystemen werden aufgrund der Rechenkomplexität in der Regel keine vollständigen Lösungsmengen berechnet. Bei größeren Systemen ist die Berechnung der exakten Lösung aufgrund der Anzahl der möglichen Kombinationen nicht praktikabel. In der Praxis wird meist ein Gewichtungsverfahren verwendet, da dieses ein hinreichend gutes Ergebnis liefert und die Rechenkomplexität auf ein ähnliches Niveau wie bei einem klassischen Knapsack-Problem reduziert.

\subsubsection{Multiobjektive Multi-Knapsack-Probleme}

Typische Scheduling Probleme in verteilten Systemen beinhalten sowohl mehrere Rechensysteme (Kapazitäten) als auch mehrere Ziele, auf die hin optimiert wird. Scheduling-Algorithmen müssen daher Multiobjektive Multi-Knapsack-Probleme lösen. Dies kann durch die Erweiterung von Multi-Knapsack Algorithmen mit Methoden für multiobjektive Knapsack-Probleme erreicht werden. Im folgenden Abschnitt wird eine beispielhafte Implementierung für einen Greedy-Algorithmus mit zwei Optimierungszielen vorgestellt:
...
Um die in dieser Arbeit priorisierte Maximierung der Rechenleistung zu erreichen, wird eine spezifische Kostenfunktion definiert. Der Greedy-Algorithmus berechnet für jeden Task $t$ und jeden Kandidaten-Knoten $n$ einen Eignungswert. Für die Strategie \enquote{Greedy (TOPS Prio)} berechnet sich der Score $S$ primär aus der verfügbaren Rechenleistung, unter der Nebenbedingung, dass Speicher-Constraints eingehalten werden:

\begin{equation}
    S(n, t) = 
    \begin{cases} 
      \frac{C_{avail}(n)}{C_{max}(n)} \cdot w_{cpu} & \text{wenn } M_{avail}(n) \ge M_{req}(t) \\
      0 & \text{sonst}
   \end{cases}
   \label{eq:scoring_function}
\end{equation}

Dabei ist $C_{avail}$ die verfügbare CPU-Leistung (in TOPS), $M_{avail}$ der freie Arbeitsspeicher und $w_{cpu}$ ein Gewichtungsfaktor. Der Algorithmus weist den Task dem Knoten mit dem höchsten Score $S$ zu. Dies stellt sicher, dass leistungsstarke Knoten bevorzugt genutzt werden, solange sie über genügend Speicher verfügen.
...
Dynamische Scheduling kann auch durch das periodische Aufrufen und neu Schedulen der Tasks stattfinden. Klassischerweise werden bei dieser Methode alle Tasks neu verteilt, so dass die Update Perioden sinnvoll gewählt werden müssen. 
...
\label{sec:MethodeZusammenfassung}

In diesem Kapitel wurde das methodische und architektonische Konzept für die verteilte Rechenplattform erarbeitet. Die Systemarchitektur folgt einem hierarchischen Ansatz, bestehend aus dezentralen Runtime Node-Komponenten auf den Fahrzeugsteuergeräten und einer zentralen Runtime Master Instanz zur Orchestrierung.

Als technisches Fundament für die Virtualisierung wurde die containerbasierte Isolierung mittels Linux-Namespaces spezifiziert, da diese eine hohe Portierbarkeit bei geringem Ressourcen-Overhead gewährleistet. Die Kommunikation im volatilen Fahrzeugnetzwerk wird durch das datenzentrierte Publish/Subscribe-Protokoll \gls{DDS} realisiert, welches durch Mechanismen zur Fehlererkennung und Authentifizierung ergänzt wurde.

Für die Ressourcenverteilung wurden verschiedene theoretische Ansätze des Knapsack-Problems analysiert. Aufgrund der Anforderungen an die Echtzeitfähigkeit und Skalierbarkeit wurde der Algorithmus \enquote{Greedy (TOPS Prio)} als präferierte Scheduling-Strategie identifiziert und mathematisch formalisiert.

Basierend auf diesen konzeptionellen Festlegungen erfolgt im nächsten Kapitel die technische Implementierung der Komponenten sowie die simulative Evaluation der Scheduling-Effizienz.