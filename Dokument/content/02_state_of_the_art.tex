...
\begin{itemize}
	\item Mount Namespace: Isoliert die Einhängenamensräume oder Mounts. Prozesse können nur auf Dateipfade zugreifen, die in der entsprechenden Mount-Datei des Systems definiert sind. Durch die Isolation erhält der Prozess eine eigene Kopie des Mount-Dateis, die sich bei Änderungen nur auf den Prozess und nicht auf das gesamte Hostsystem auswirkt.
	...
	\item PID Namespace: Prozesse im Container haben eigene Prozessidentifikationsnummern (\gls{PID}). Somit kann in jedem Container ein Initialisierungsprozess mit \gls{PID} 1 vorkommen. Ein weiterer Effekt der \gls{PID} Isolierung ist, dass jeder Container einen eigenen /proc Ordner erhält, das bedeutet dass Container nur ihre eigene Prozesse einsehen können, die restlichen Prozesse im System sind von innerhalb des Containers unsichtbar. Das System selbst kann die Containerprozesse sehen, allerdings haben diese Containerprozesse aus Systemsicht andere \gls{PID}s als innerhalb des Containers.
...
\label{Orchestrierung Stand der Technik}
...
\begin{itemize}
	\item NVIDIA Dive AGX Thor: 1000 TOPS (8 Bit)/2000 TOPS (4 Bit) \cite{NVIDIA2025}
	\item NVIDIA Dive AGX Orin: 254 Tops (8 Bit) \cite{NVIDIA2025}
...
Mehrere Automobilhersteller setzen bereits auf dem Open Source Betriebssystem Linux für deren leistungsfähige Steuergeräte: BMW \cite{Foss2019}, Tesla \cite{Tesla2024} und Toyota \cite{endgadget2017} verwenden jeweils ein entsprechend angepasstes Linux Betriebssystem.
